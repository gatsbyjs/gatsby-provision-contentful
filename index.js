...
function provisionSpace({ spaceId, managementToken }) {
  const data = require(path.resolve(
    process.env.INIT_CWD,
    argv.contentfulDataPath
  ));
...
function main() {
  if (process.env.GATSBY_CLOUD === "true") {
    ...
    return provisionSpace({
      spaceId: argv.spaceId || process.env.CONTENTFUL_SPACE_ID,
      managementToken:
        argv.managementToken || process.env.CONTENTFUL_MANAGEMENT_TOKEN,
    })
      .then(() => console.log("All finished! Your space is now provisioned."))
      .catch((e) => {
        console.error(`Error provisioning Contentful space: ${e}`);
        process.exit(1);
      });
  } else {
    // we're running this script locally
    console.log(`
      ...
      .then(
        ({
          ...
          const configFiles = [`.env.development`, `.env.production`].map(
            (file) => path.join(process.env.INIT_CWD, file)
          );
